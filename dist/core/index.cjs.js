...
var _3dTilesRenderer = require('3d-tiles-renderer');
...
class TilesViewer {
  ...
  _initScene() {
    ...
    this.scene.background = new THREE__namespace.Color(0x111122);
    this.scene.fog = new THREE__namespace.FogExp2(0x111122, 0.0003);
    ...
    const gridHelper = new THREE__namespace.GridHelper(200, 20, 0x88aaff, 0x335588);
    gridHelper.position.y = -1;
    this.scene.add(gridHelper);

    const axesHelper = new THREE__namespace.AxesHelper(50);
    this.scene.add(axesHelper);

    const originSphere = new THREE__namespace.Mesh(
      new THREE__namespace.SphereGeometry(0.5, 16, 16),
      new THREE__namespace.MeshStandardMaterial({ color: 0xff3333, emissive: 0x330000 })
    );
    this.scene.add(originSphere);

    const ambientLight = new THREE__namespace.AmbientLight(0xffffff, 0.65);
    this.scene.add(ambientLight);
    const hemiLight = new THREE__namespace.HemisphereLight(0x8bb5ff, 0x3b2a1f, 0.9);
    this.scene.add(hemiLight);
    const mainLight = new THREE__namespace.DirectionalLight(0xfff5e6, 1.4);
    mainLight.position.set(6, 12, 8);
    mainLight.castShadow = true;
    this.scene.add(mainLight);
    const backLight = new THREE__namespace.DirectionalLight(0x88aaff, 0.6);
    backLight.position.set(-4, 5, -6);
    this.scene.add(backLight);
    const rightFill = new THREE__namespace.DirectionalLight(0xffaa88, 0.5);
    rightFill.position.set(4, 3, 5);
    this.scene.add(rightFill);
    const fillPoint = new THREE__namespace.PointLight(0xffaa66, 0.35);
    fillPoint.position.set(2, 3, 2);
    this.scene.add(fillPoint);
    const leftFill = new THREE__namespace.DirectionalLight(0xaaccff, 0.4);
    leftFill.position.set(-3, 4, 3);
    this.scene.add(leftFill);
  }

  _initTiles(tilesetUrl) {
    this.tilesRenderer = new _3dTilesRenderer.TilesRenderer(tilesetUrl);
  ...
  _fitCamera() {
    ...
    if (sphere && sphere.radius > 0 && !isNaN(sphere.center.x)) {
      ...
      const verticalFov = THREE__namespace.MathUtils.degToRad(this.camera.fov);
      const horizontalFov = 2 * Math.atan(Math.tan(verticalFov / 2) * this.camera.aspect);
      const fitFov = Math.min(verticalFov, horizontalFov);
      const distance = (sphere.radius / Math.sin(fitFov / 2)) * fitOffset;
      const direction = new THREE__namespace.Vector3(1, 0.3, 1).normalize();
...
exports.TilesViewer = TilesViewer;
